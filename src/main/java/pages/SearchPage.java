...
public class SearchPage extends Common
{
	...
	public void clickOnPreviewOnSP(AppiumDriver<MobileElement> driver, String searchString)
	{
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		try
		{
	...
	public boolean VerifyPreviewCollectorLayerIsDisplayedOnSP(AppiumDriver<MobileElement> driver) {
		...
		try {
			...
			if (driver.findElement(By.xpath("(//*[@text='Preview Collector Layer'])")).isDisplayed()) {
				...
				System.out.println("Preview collector page is displayed for " + clientName + " client name");
				record = true;
			}
